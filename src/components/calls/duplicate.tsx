...
import { Phone, PhoneIncoming, PhoneOutgoing, PhoneMissed, XCircle, Download, Filter, Search, Clock, Play, Pause, SkipBack, SkipForward, Mic, FileText, Calendar, Tag } from 'lucide-react';
...
const CallHistory = () => {
  ...
  const fetchCalls = async () => {
    try {
      const response = await axios.get('https://caller-crm-api.justvy.in/call/?agentId=AGENT046');
      console.log('API response:', response.data);

      const apiCalls = Array.isArray(response.data)
        ? response.data
        : response.data.calls || response.data.data || [];

      if (!Array.isArray(apiCalls)) {
        throw new Error('Unexpected API response structure');
      }

      const uniqueAgentIds = [...new Set(apiCalls.map(call => call.agentId).filter(Boolean))];

      const agentResponses = await Promise.all(
        uniqueAgentIds.map(id =>
          axios.get(`https://caller-crm-api.justvy.in/agent/${id}`)
            .then(res => ({ id, name: res.data.agent.name }))
            .catch(() => ({ id, name: 'Unknown Agent' })) // fallback
        )
      );

      const agentMap: Record<string, string> = {};
      agentResponses.forEach(({ id, name }) => {
        agentMap[id] = name;
      });

      const formattedCalls = apiCalls.map((call: any) => ({
        id: call.id,
        name: call.name || 'Unknown',
        number: call.mobileNumber,
        type: call.callType?.toLowerCase() || 'incoming',
        status:  call.callType?.toLowerCase() || 'unknown',
        duration: call.duration
          ? `${Math.floor(call.duration / 60)}:${String(call.duration % 60).padStart(2, '0')}`
          : '',
        date: call.dateTime?.split('T')[0] || '',
        time: call.dateTime?.split('T')[1]?.substring(0, 5) || '',
        sentiment: 'neutral',
        hasRecording: !!call.recording_url,
        hasTranscript: false,
        notes: call.comment || '',
        tags: [],
        agent: agentMap[call.agentId] || '-',
